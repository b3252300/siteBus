...
(async () => {
  ...
  function getAuthorizationHeader() {
    const AppID = process.env.TDX_APP_ID || process.env.VITE_TDX_APP_ID || 'b32523-bc1a72b3-d0f5-4a88';
    const AppKey = process.env.TDX_APP_KEY || process.env.VITE_TDX_APP_KEY || '1ef51d3c-7adf-4993-929a-4b2f5cf445cd';
    const GMTString = new Date().toGMTString();
    const ShaObj = new jsSHA('SHA-1', 'TEXT');
    ShaObj.setHMACKey(AppKey, 'TEXT');
    ShaObj.update('x-date: ' + GMTString);
